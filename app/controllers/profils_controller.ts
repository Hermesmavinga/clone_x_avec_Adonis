...
export default class ProfilesController {
   ...
   */
  public async myProfile({ auth, view, request }: HttpContext) {
    // 🔥 AJOUT: Récupérer le type de liste depuis les query params
    const listType = request.qs().list // 'followers' ou 'following'

    const user = await User.query()
      .where('id', auth.user!.id)
      .preload('followers', (query) => {
        query.preload('follower') // 🔥 PRÉCHARGER le follower
      })
      .preload('following', (query) => {
        query.preload('followed') // 🔥 PRÉCHARGER le followed
      })
      .firstOrFail()
    ...
    return view.render('pages/profil', {
      ...
      suggestions,
      listType, // 🔥 ENVOYER listType au template
    })
  }
  ...
  private async getUserSuggestions(currentUser: User) {
    ...
    const suggestions = await User.query()
      ...
      .limit(5)
      .select('id', 'fullname') // 🔥 AJOUTER username
    ...
    const suggestionsWithFollowStatus = await Promise.all(
      suggestions.map(async (user) => {
        ...
        return {
          id: user.id,
          fullname: user.fullname, // 🔥 AJOUTER username
  ...
  public async showUserProfile({ params, view, auth, request }: HttpContext) {
    // 🔥 AJOUT: Récupérer le type de liste
    const listType = request.qs().list
